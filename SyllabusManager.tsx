    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-24 md:pb-20">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-2 md:gap-3">
          <button onClick={() => fileInputRef.current?.click()} disabled={isProcessing} className="group flex flex-1 sm:flex-none items-center justify-center gap-2 px-4 py-3 md:px-6 md:py-3.5 bg-white dark:bg-slate-900 text-emerald-600 dark:text-emerald-400 border-2 border-emerald-100 dark:border-emerald-900 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-sm hover:bg-emerald-50 transition-all disabled:opacity-50">
            {isProcessing ? <Loader2 className="animate-spin" size={16} /> : <Camera size={16} />}
            {t('সিলেবাস স্ক্যান', 'Scan Syllabus')}
          </button>
          
          <button onClick={() => routineFileInputRef.current?.click()} disabled={isRoutineProcessing} className="group flex flex-1 sm:flex-none items-center justify-center gap-2 px-4 py-3 md:px-6 md:py-3.5 bg-white dark:bg-slate-900 text-blue-600 dark:text-blue-400 border-2 border-blue-100 dark:border-blue-900 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-sm hover:bg-blue-50 transition-all disabled:opacity-50">
            {isRoutineProcessing ? <Loader2 className="animate-spin" size={16} /> : <FileText size={16} />}
            {t('রুটিন আপলোড', 'Upload Routine')}
          </button>
          
          <div className="flex gap-2 w-full sm:w-auto">
            <button onClick={() => { setShowManualAdd(!showManualAdd); setShowManualRoutine(false); }} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 md:px-6 md:py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all ${showManualAdd ? 'bg-emerald-500 text-white' : 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900'}`}>
              <Plus size={16} />
              {t('ম্যানুয়াল অ্যাড', 'Manual Add')}
            </button>
            <button onClick={() => { setShowManualRoutine(!showManualRoutine); setShowManualAdd(false); }} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-3 md:px-6 md:py-3.5 rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-lg active:scale-95 transition-all ${showManualRoutine ? 'bg-indigo-600 text-white' : 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 border border-indigo-100 dark:border-indigo-800'}`}>
              <Calendar size={16} />
      ...
      {showManualAdd && (
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 border-dashed border-emerald-200 dark:border-emerald-800 animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <h3 className="font-black text-lg md:text-xl flex items-center gap-3"><Plus className="text-emerald-500" />{t('নতুন বিষয় যোগ করো', 'Add New Subject')}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6">
            <div className="space-y-2">
              ...
              <input type="text" value={manualSubject.name} onChange={e => setManualSubject({...manualSubject, name: e.target.value})} placeholder="e.g. Physics" className="w-full px-5 py-3 md:px-6 md:py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-emerald-500 rounded-2xl font-bold outline-none" />
            </div>
            <div className="space-y-2">
              <label className="text-[9px] md:text-[10px] font-black uppercase text-slate-400 tracking-widest ml-1">{t('পত্র', 'Paper')}</label>
              <select value={manualSubject.paper} onChange={e => setManualSubject({...manualSubject, paper: parseInt(e.target.value)})} className="w-full px-5 py-3 md:px-6 md:py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-emerald-500 rounded-2xl font-bold outline-none appearance-none">
          ...
          <div className="space-y-2 mb-8 md:mb-10">
            ...
            <textarea value={manualSubject.chapters} onChange={e => setManualSubject({...manualSubject, chapters: e.target.value})} placeholder="Chapter 1&#10;Chapter 2" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-emerald-500 rounded-2xl font-bold min-h-[120px] outline-none" />
          </div>
          <button onClick={handleManualAdd} className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-black py-4 rounded-2xl shadow-xl shadow-emerald-500/10 uppercase tracking-widest text-xs transition-all">{t('সেভ করো', 'Add Subject')}</button>
        </div>
      )}
      ...
      {showManualRoutine && (
        <div className="bg-white dark:bg-slate-900 p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border-2 border-dashed border-indigo-200 dark:border-indigo-800 animate-in zoom-in-95 duration-300">
          <div className="flex items-center justify-between mb-6 md:mb-8">
            <h3 className="font-black text-lg md:text-xl flex items-center gap-3"><Calendar className="text-indigo-500" />{t('রুটিন ম্যানুয়াল এন্ট্রি', 'Manual Routine Entry')}</h3>
          ...
          <div className="space-y-4 mb-8">
            <div className="bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800/30">
               ...
               <p className="text-[10px] text-indigo-600/70 dark:text-indigo-400/60 leading-relaxed font-medium">{t('প্রতি লাইনে একটি বিষয়, পত্র এবং তারিখ (YYYY-MM-DD) লিখো।', 'Enter subject, paper, and date (YYYY-MM-DD) per line.')}</p>
            </div>
            <textarea value={bulkRoutine} onChange={e => setBulkRoutine(e.target.value)} placeholder="Physics 1 2025-05-20&#10;Chemistry 2 2025-06-15" className="w-full px-5 py-4 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-indigo-500 rounded-2xl font-bold min-h-[150px] outline-none" />
          </div>
          <button onClick={handleBulkRoutineAdd} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl shadow-xl shadow-indigo-600/10 uppercase tracking-widest text-xs transition-all">{t('রুটিন সেভ করো', 'Save Routine')}</button>
        </div>
      )}
              ...
              <div className="p-6 md:p-10 pb-0">
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-6 md:mb-8">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 md:gap-4 mb-3">
                      <div className={`w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-[1.5rem] flex items-center justify-center text-white font-black text-lg md:text-xl shadow-lg ${progressPercent === 100 ? 'bg-emerald-500 shadow-emerald-500/20' : 'bg-slate-900 dark:bg-slate-700'}`}>{sub.name[0]}</div>
                      <div>
                        <h3 className="font-black text-xl md:text-2xl tracking-tight flex items-center gap-2">{sub.name}{progressPercent === 100 && <CheckCircle size={20} className="text-emerald-500" />}</h3>
                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[9px] md:text-[10px] font-black uppercase rounded-md tracking-widest">{t('পত্র', 'P')} {sub.paper}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="relative mr-2 hidden sm:block">
                      ...
                      <input type="text" placeholder={t("চ্যাপ্টার খুঁজুন...", "Find chapter...")} value={chapterSearch[sub.id] || ''} onChange={(e) => setChapterSearch({...chapterSearch, [sub.id]: e.target.value})} className="pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-800 border-0 rounded-xl text-[10px] font-bold w-40 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all" />
                    </div>
                    <button onClick={() => toggleAllChapters(sub.id, completedCount < totalCount)} className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-black text-[9px] uppercase tracking-widest transition-all ${completedCount === totalCount ? 'bg-slate-100 dark:bg-slate-800 text-slate-400' : 'bg-emerald-500 text-white shadow-lg'}`}>{completedCount === totalCount ? t('মুছুন', 'Reset') : t('শেষ', 'Done')}</button>
                    <button onClick={() => deleteSubject(sub.id)} className="p-2.5 text-slate-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/10 transition-all rounded-xl opacity-100 md:opacity-0 md:group-hover:opacity-100"><Trash2 size={18} /></button>
                  </div>
                </div>
                <div className="space-y-2 md:space-y-3 mb-8 md:mb-10">
                  ...
                  <div className="h-3 md:h-4 w-full bg-slate-50 dark:bg-slate-800 rounded-full overflow-hidden p-0.5 shadow-inner">
                    <div className="h-full rounded-full bg-gradient-to-r from-emerald-500 to-teal-300 transition-all duration-1000" style={{ width: `${progressPercent}%` }} />
                  </div>
                </div>
              </div>
                    ...
                    <div key={ch.id} className={`group/item flex flex-col p-4 md:p-6 rounded-[2rem] transition-all border-2 bg-white dark:bg-slate-900 ${ch.isCompleted ? 'border-emerald-50 dark:border-emerald-900/30' : 'border-transparent hover:border-slate-100 dark:hover:border-slate-700 shadow-sm'}`}>
                      <div className="flex items-start gap-3 mb-4">
                        <div className="relative pt-1 shrink-0">
                          ...
                          <CheckCircle2 size={12} className="absolute top-[10px] left-[4px] md:top-[11px] md:left-[5px] text-white opacity-0 peer-checked:opacity-100 transition-opacity pointer-events-none" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <span className={`font-bold text-xs md:text-sm leading-tight block ${ch.isCompleted ? 'text-emerald-700 dark:text-emerald-400 line-through opacity-60' : 'text-slate-600 dark:text-slate-200'}`}>{ch.name}</span>
                          {ch.studyTimeSeconds && ch.studyTimeSeconds > 0 && (
                            <div className="mt-1 flex items-center gap-1.5 text-[9px] font-black text-brand-primary uppercase tracking-tighter bg-brand-primary/5 px-2 py-0.5 rounded-full w-fit">
                              <Clock size={10} /> {formatDuration(ch.studyTimeSeconds)}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      {ch.isCompleted && (
                        <button 
                          ...
                          className="mb-4 w-full py-2 bg-emerald-500 text-white rounded-xl text-[9px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/20 animate-in fade-in slide-in-from-top-2"
                        >
                      ...
                      <div className="mt-auto flex items-center justify-between border-t border-slate-50 dark:border-slate-800 pt-3">
                         <div className="flex gap-1.5">
                            {(['Easy', 'Medium', 'Hard'] as Difficulty[]).map((level) => (
                              <button key={level} onClick={() => updateDifficulty(sub.id, ch.id, level)} className={`w-6 h-6 rounded-full flex items-center justify-center transition-all ${ch.difficulty === level ? `${difficultyMeta[level].bgColorClass} text-white shadow-md scale-110` : 'bg-slate-50 dark:bg-slate-800 text-slate-300 hover:text-slate-500'}`} title={t(difficultyMeta[level].label.bn, difficultyMeta[level].label.en)}><Tag size={10} /></button>
                            ))}
                         </div>
