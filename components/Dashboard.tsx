...
const Dashboard: React.FC<DashboardProps> = ({ userState }) => {
  ...
  const moodStyles = {
    'Great': 'text-brand-primary bg-brand-primary/10',
    'Focused': 'text-brand-secondary bg-brand-secondary/10',
    'Tired': 'text-orange-500 bg-orange-500/10',
  ...
  return (
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-700">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black tracking-tight text-brand-text-p">
            {userState.profile?.fullName}! {t('তোমার ড্যাশবোর্ড', 'Your Dashboard')}
          </h1>
          <p className="text-brand-text-s font-medium text-xs md:text-sm mt-1">{t('তোমার HSC প্রস্তুতির বর্তমান চিত্র।', 'Your current HSC readiness overview.')}</p>
        </div>
        <div className={`px-4 py-2 md:px-5 md:py-2.5 rounded-2xl flex items-center gap-2 md:gap-3 border border-brand-text-s/10 shadow-sm transition-all ${moodStyles} self-start md:self-center`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 bg-brand-surface p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] shadow-sm border border-brand-text-s/10 relative overflow-hidden transition-colors">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 md:mb-10 relative z-10 gap-4">
            <div>
              <h3 className="text-[10px] font-black text-brand-text-s uppercase tracking-[0.25em] mb-1 md:mb-2">{t('প্রস্তুতি লেভেল', 'Readiness Level')}</h3>
              <div className="flex items-baseline gap-2">
                <h2 className="text-5xl md:text-7xl font-black text-brand-text-p leading-none tracking-tighter">{stats.score}</h2>
                <span className="text-xl md:text-2xl font-bold text-brand-text-s/30">%</span>
              </div>
            </div>
            <div className="sm:text-right flex flex-col items-start sm:items-end">
              <div className="inline-flex items-center gap-2 px-4 py-2 md:px-5 md:py-2.5 rounded-2xl bg-brand-primary text-white shadow-xl shadow-brand-primary/30 mb-2 md:mb-3 transform hover:scale-105 transition-transform cursor-default">
              ...
              <p className="text-[9px] md:text-[10px] text-brand-text-s font-black uppercase tracking-[0.1em]">{t('NCTB প্রোটোকল অনুযায়ী', 'NCTB Compliant Scoring')}</p>
            </div>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 md:gap-x-16 gap-y-6 md:gap-y-8 relative z-10">
            {[
              { label: t('পড়াশোনা (৪০%)', 'Study (40%)'), value: stats.factors.study, color: 'from-brand-primary to-brand-primary/60' },
              { label: t('সিলেবাস (৩০%)', 'Syllabus (30%)'), value: stats.factors.completion, color: 'from-brand-secondary to-brand-secondary/60' },
              ...
              <div key={idx} className="space-y-2 md:space-y-3">
                ...
                <div className="h-2 md:h-2.5 w-full bg-brand-bg rounded-full overflow-hidden shadow-inner">
          ...
          <div className="mt-8 md:mt-10 pt-6 md:pt-8 border-t border-brand-text-s/10 flex items-center gap-3 md:gap-4 relative z-10">
             <div className="p-2 bg-brand-bg rounded-xl">
               <Info size={16} md:size={18} className="text-brand-text-s" />
             </div>
             <p className="text-[9px] md:text-[11px] text-brand-text-s font-bold leading-relaxed italic uppercase tracking-tighter">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-6 md:gap-8">
          <div className="bg-brand-primary text-white p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] shadow-2xl shadow-brand-primary/20 relative overflow-hidden group">
          ...
          <div className="bg-brand-surface p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-brand-text-s/10 shadow-sm relative group transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        <div className="lg:col-span-2 bg-brand-surface p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] border border-brand-text-s/10 shadow-sm overflow-hidden transition-colors">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6 mb-8 md:mb-10">
            <div>
              <h4 className="text-sm font-black text-brand-text-s flex items-center gap-2 md:gap-3 uppercase tracking-[0.2em]">
                <BarChart3 size={18} md:size={20} className="text-brand-primary" /> {t('সাপ্তাহিক রিপোর্ট', 'Weekly Report')}
              </h4>
            </div>
          </div>
        ...
        <div className="bg-brand-surface p-6 md:p-8 rounded-[2rem] md:rounded-[3rem] border border-brand-text-s/10 shadow-sm flex flex-col transition-colors">
          <div className="flex items-center gap-3 mb-6 md:mb-8">
            <History size={18} className="text-brand-secondary" />
            <h4 className="text-xs md:text-sm font-black text-brand-text-p uppercase tracking-widest">{t('সাম্প্রতিক হিস্ট্রি', 'History')}</h4>
          </div>
          
          <div className="flex-1 space-y-4 overflow-y-auto pr-1 max-h-[300px] md:max-h-[380px] scrollbar-hide">
            {recentSessions.length > 0 ? recentSessions.map((session) => (
              <div key={session.id} className="group flex items-start gap-3 p-3 rounded-[1.25rem] bg-brand-bg/50 hover:bg-brand-bg border border-transparent hover:border-brand-text-s/10 transition-all">
                <div className={`mt-0.5 w-8 h-8 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${session.isRevision ? 'bg-brand-secondary/20 text-brand-secondary' : 'bg-brand-primary/20 text-brand-primary'}`}>
                  {session.isRevision ? <RefreshCcw size={14} /> : <BookOpen size={14} />}
                </div>
                <div className="flex-1 min-w-0">
                  <h5 className="font-black text-[10px] text-brand-text-p truncate mb-0.5">
                    {getSubjectName(session.subjectId)}
                  </h5>
                  <div className="flex items-center gap-2 text-[9px] font-bold text-brand-text-s">
                    <span className="flex items-center gap-1"><Clock size={10} /> {session.durationMinutes}m</span>
                    <span className="flex items-center gap-1 text-brand-secondary"><Coffee size={10} /> {session.breakMinutes || 0}m</span>
                  </div>
                </div>
              </div>
            )) : (
              <p className="text-center text-[10px] font-bold text-brand-text-s py-8">{t('এখনো কোনো সেশন নেই!', 'No history yet!')}</p>
            )}
          </div>
        </div>
      </div>
