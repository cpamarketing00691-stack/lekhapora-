      ...
      <nav className="sticky top-0 z-[100] w-full bg-brand-bg/80 backdrop-blur-md border-b border-brand-text-s/10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-10">
            {navLinks.map((link) => (
              <Link key={link.path} to={link.path} className={`text-sm font-bold transition-colors ${location.pathname === link.path ? 'text-brand-primary' : 'text-brand-text-s hover:text-brand-primary'}`}>
                {link.name}
              </Link>
            ))}
          ...
          <button className="md:hidden text-brand-text-p" onClick={() => setIsOpen(!isOpen)}>{isOpen ? <X /> : <Menu />}</button>
        </div>
      ...
      <footer className="bg-brand-surface border-t border-brand-text-s/10 pt-20 pb-10 mt-auto relative z-10">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
          <div className="col-span-1 md:col-span-2 space-y-6">
            ...
            <p className="text-brand-text-s font-medium max-w-sm">Empowering students in Bangladesh through smart digital preparation and structured study frameworks.</p>
          </div>
          <div>
