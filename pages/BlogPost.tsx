...
const BlogPost: React.FC = () => {
  ...
  if (!post) {
    return (
      <div className="min-h-screen bg-[#171717] flex flex-col items-center justify-center p-6 text-center font-sans">
        <h1 className="text-4xl font-bold text-white mb-4">Post Not Found</h1>
        <p className="text-gray-400 mb-8">The article you are looking for does not exist or has been removed.</p>
        <Link to="/blog" className="px-6 py-3 bg-purple-500 text-white rounded-xl font-medium hover:bg-purple-500/90 transition-colors">
          Back to Blog
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-[#171717] py-12 font-serif">
      <div className="max-w-4xl mx-auto px-6">
        <button onClick={() => navigate('/blog')} className="inline-flex items-center gap-2 text-gray-500 hover:text-purple-400 mb-8 font-sans font-medium transition-colors">
          <ArrowLeft size={16} /> Back to Articles
        </button>

        <article className="bg-[#0a0a0a] p-8 md:p-12 rounded-3xl shadow-sm border border-white/5">
          <header className="mb-10 text-center border-b pb-10">
            <div className="flex items-center justify-center gap-4 text-sm text-gray-500 font-sans mb-6 uppercase tracking-wider">
              ...
              <span>•</span>
              <span className="text-purple-400 font-bold">{post.category || 'Uncategorized'}</span>
            </div>
            <h1 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-6">{post.title}</h1>
            <div className="flex items-center justify-center gap-3 font-sans text-gray-400">
          ...
          <div className="prose prose-lg max-w-none text-gray-700 font-sans">
            {post.blocks.map((block, idx) => {
              if (block.type === 'heading') return <h2 key={idx} className="text-3xl font-bold text-white mt-8 mb-4 font-serif">{block.content}</h2>;
              ...
              if (block.type === 'divider') return <hr key={idx} className="my-10 border-white/10" />;
              return null;
            })}
          </div>
          ...
          <footer className="mt-12 pt-8 border-t flex flex-wrap gap-2 font-sans">
             ...
             {['HSC', 'Preparation', post.category || 'General'].map(tag => (
                <span key={tag} className="px-3 py-1 bg-gray-50 border text-sm text-gray-400 rounded-full">{tag}</span>
             ))}
          </footer>
        </article>
      </div>
